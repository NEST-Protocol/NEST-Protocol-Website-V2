...
const Page = () => {
  ...
  const pcPage = (
    <Stack bgImage={'/image/Oracle/bg.jpg'} bgPosition={"center"} bgSize={'cover'}>
      ...
      <Stack textAlign={"center"} align={"center"} spacing={'30px'} py={'120px'}>
        ...
        <Text fontSize={'18px'} fontWeight={'400'} color={'rgba(3, 3, 8, 0.6)'}>Unlike the price generated by Price Feed
          in the market, <br/>
        ...
        <Wrap spacing={'44px'} w={'1100px'} justify={"center"}>
          {
            array1.map((item, index) => (
              <WrapItem key={index}>
                <Stack w={'308px'} bg={'rgba(255,255,255,0.8)'} borderRadius={'20px'} py={'72px'}
                       boxShadow={'0px 0px 45px 5px #E5E5E5'} spacing={'40px'} border={'1px solid #EEEEEE'}>
                  <Stack h={'80px'}>
            ...
            array2.map((item, index) => (
              <WrapItem key={index}>
                <Stack w={'308px'} h={'300px'} bg={'white'} borderRadius={'20px'} px={'30px'} justify={"center"}>
                  ...
                  <Link color={'#EAAA00'} fontWeight={'600'} fontSize={'15px'} href={item.link}>
              ...
              <WrapItem key={index}>
                <Stack w={'308px'} bg={'white'} borderRadius={'20px'}>
                  ...
                  <Stack px={'30px'} py={'24px'} h={'240px'}>
                    ...
                    <Text fontSize={'13px'} fontWeight={'400'} color={'rgba(3, 3, 8, 0.6)'}>{item.desc}</Text>
                    <Spacer/>
                    <Text fontSize={'13px'} fontWeight={'400'} color={'rgba(3, 3, 8, 0.6)'}>{item.time}</Text>
                  </Stack>
                </Stack>
              </WrapItem>
            ))
  ...
  const mobilePage = (
    <Stack bgImage={'/image/Oracle/bg.jpg'} bgPosition={"center"} bgSize={'cover'}>
      {SEO}
      <NavigationMobile/>
      <Stack textAlign={"center"} align={"center"} py={'90px'} px={'20px'}>
        <Heading fontSize={'25px'} fontWeight={'bold'}>NEST Oracle,<br/>
          The Truly<br/>
          Decentralized Oracle
        </Heading>
        <Text fontSize={'12.5px'} fontWeight={'400'} color={'rgba(3, 3, 8, 0.6)'}>Unlike the price generated by Price
          Feed in the market, <br/>it constructs a
          decentralized non-cooperative gaming
          network and uses the mechanism
          of arbitrage and two-way options to
          determine the final price. <br/>
          NEST Oracle also introduces random
          information based on decentralized
        ...
        <Wrap w={'full'} spacing={'20px'} justify={'center'}>
          {
            array1.map((item, index) => (
              <WrapItem key={index} w={'44%'} borderRadius={'20px'}>
                <Stack bg={'rgba(255,255,255,0.8)'} py={'35px'} w={'full'} borderRadius={'20px'}
                       border={'1px solid #EEEEEE'} spacing={'20px'} justify={"center"}>
                  <Stack h={'40px'}>
                    <chakra.img src={item.img} h={'full'} objectFit={'contain'}/>
                  </Stack>
      ...
      <Stack py={'62px'} px={'24px'} spacing={'24px'}>
        {
          array2.map((item, index) => (
            <Stack key={index} px={'38px'} py={'100px'} borderRadius={'20px'} boxShadow={'0px 0px 45px 5px #E5E5E5'}
              ...
              <Text fontSize={'12.5px'} fontWeight={'400'} color={'rgba(3, 3, 8, 0.6)'}>{item.desc}</Text>
              <Stack h={'30px'}>
              </Stack>
              <Link href={item.link} color={'#00A0E9'} fontSize={'12.5px'} fontWeight={600}>{item.linkText}
          ...
          array3.map((item, index) => (
            <Stack w={'full'} bg={'white'} borderRadius={'20px'} key={index} boxShadow={'0px 0px 45px 5px #E5E5E5'}>
              ...
              <Stack px={'30px'} py={'24px'}>
                ...
                <Text fontSize={'10px'} fontWeight={'400'} color={'rgba(3, 3, 8, 0.6)'}>{item.desc}</Text>
                <Text fontSize={'10px'} fontWeight={'400'} color={'rgba(3, 3, 8, 0.6)'}>{item.time}</Text>
              </Stack>
            </Stack>
          ))
        }
